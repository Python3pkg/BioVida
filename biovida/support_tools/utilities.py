...
def _train_val_test_error_checking(target_dir, action, group_files_dict, tvt, allowed_actions):
    ...
    min_number_of_files = len(list(tvt.keys())) * len(list(group_files_dict.keys()))
    for k, v in list(group_files_dict.items()):
        if len(v) < min_number_of_files:
            raise InsufficientNumberOfFiles("\nThe '{0}' subdirectory/subgroup only contains {1} files,\n"
                                            ...
                                            k, len(v), min_number_of_files, ", ".join(list(tvt.keys())),
                                            ", ".join(list(group_files_dict.keys()))))
...
def _train_val_test_dict_sort(tvt):
    ...
    return sorted(list(tvt.items()), key=lambda x: order_dict.get(x[0]))
...
def _list_divide(l, tvt, random_state):
    ...
    for e, (k, v) in enumerate(tvt_sorted, start=1):
        ...
        right_full = int(left) + int(right) if e != len(list(tvt.keys())) else len(l_shuffled)
...
def _file_paths_dict_to_ndarrays(dictionary, dimensions, stack, verbose=True):
    ...
    def values_to_ndarrays(d):
        return {k2: load_as_ndarrays(v2) for k2, v2 in status_inner(list(d.items()))}

    if dimensions == 1:
        return values_to_ndarrays(dictionary)
    elif dimensions == 2:
        return {k: values_to_ndarrays(v) for k, v in status_outer(list(dictionary.items()))}
...
def _train_val_test_engine(action, tvt, group_files_dict, target_path, random_state, allowed_actions, verbose):
    ...
    output_dict = dict()
    for k, v in tqdm(list(group_files_dict.items()), desc='Divvying Groups', disable=not verbose):
...
def _tvt_dict_gen(d):
    ...
    return {k: v for k, v in list(d.items()) if k in _TVT_GROUPS and is_numeric(v)}
