...
def _directory_creator(cache_path=None, verbose=True):
    ...
    # Record Created Dirs
    created_dirs += list({k: v for k, v in list(sub_dirs_made.items()) if k[1] is True}.values())
...
def package_cache_creator(sub_dir, to_create, cache_path=None, nest=None, verbose=True, requires_medpix_logo=False):
    ...
    # New dirs created
    new = {k: v for k, v in list(package_created_dirs.items()) if k[1] is True}

    # Print record of files created, if verbose is True
    if verbose and len(list(new.values())):
        _created_notice(list(new.values()), root_path)

    # Render a hash map of `cache_path` - to - local address
    record_dict = {k[0]: os.path.join(sub_dir_full_path, v.split(os.sep)[-1]) for k, v in list(package_created_dirs.items())}
    ...
    if sub_dir == 'images' and requires_medpix_logo:
        if 'aux' not in list(record_dict_nest.keys()):
