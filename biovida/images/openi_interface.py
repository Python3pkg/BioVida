...
class _OpeniSearch(object):
    ...
    def _openi_search_check(self, search_arguments):
        ...
        for k, v in list(search_arguments.items()):
            ...
            if k != 'query' and v is not None:
                ...
                for i in v:
                    ...
                    if i not in list(self.search_dict[k][1].keys()):
                        raise ValueError(general_error_msg.format(
                            i, k, list_to_bulletpoints(list(self.search_dict[k][1].keys()))))
    ...
    def _exclusions_image_type_merge(args, exclusions):
        ...
        args['image_type'] += list(['exclude_{0}'.format(x) for x in exclusions])
    ...
    def options(self, search_parameter=None, print_options=True):
        ...
        if search_parameter is None:
            ...
            opts = [i for i in list(search_dict_against_param[1].keys()) if i not in exclusions]
            if not len(opts):
    ...
    def search(self,
        ...
        args_cleaned = {k: v for k, v in list(deepcopy(locals()).items())
        ...
        args = {k: [v] if isinstance(v, str) and k != 'query' else v
                for k, v in list(args_cleaned.items()) if k != 'exclusions'}
        ...
        # Save search query
        self.current_search = {k: v for k, v in list(deepcopy(search_arguments).items()) if v is not None}
        ...
        api_search_transform = {self._api_url_param(k): self._api_url_terms(k, v)
                                for k, v in list(search_arguments.items()) if v is not None}
...
class _OpeniRecords(object):
    ...
    def harvest_vector(request_rslt):
        ...
        to_harvest = list()
        for k, v in list(request_rslt.items()):
            if isinstance(v, str):
                to_harvest.append(k)
            elif isinstance(v, dict) and any(dmy in [x.lower() for x in list(v.keys())] for dmy in time_units):
                to_harvest.append(k)
...
class _OpeniImages(object):
    ...
    def _title_image(self, url, image_size):
        ...
        replacement_terms = [cln(x) for x in (str(1), b_name, image_size, image_format.replace(".", ""))]
    ...
    def pull_images(self,
        ...
        pickle.dump({k: v for k, v in list(locals().items()) if k not in ('self', 'settings_path')},
    ...
    def _latent_temp_dir(self):
        ...
        if os.path.isdir(self._Images.temp_directory_path):
            ...
            if len(latent_pickles):
                ...
                settings_dict_for_pull = {k: v for k, v in list(settings_dict.items()) if k not in ['records_db']}
